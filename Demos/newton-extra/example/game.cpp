...
void CGame::Init()
{
	...
	device = createDevice(EDT_OPENGL, dimension2d<u32>(1024, 768), 16, false, true, this);
...
//	NewtonMaterialSetCollisionCallback (nWorld, i, i, NULL, NULL, NULL, NULL);
		NewtonMaterialSetCollisionCallback (nWorld, i, i, NULL, NULL);
...
void CGame::Prepare()
{
	...
	g_mapnode = smgr->addOctreeSceneNode(g_map->getMesh(0));
...
void CGame::SetMeshTransformEvent(const NewtonBody* body, const float* matrix)
{
...
void CGame::ApplyForceAndTorqueEvent (const NewtonBody* body)
{
   float mass;
   float Ixx;
   float Iyy;
